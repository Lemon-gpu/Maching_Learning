...
def calculate_all_infomation_gain(dataset: pd.DataFrame, label_index: int):
    ...
    result = {}
    for i in columns:
        result[i] = calculate_information_gain(
            dataset, i, dataset.columns[label_index])
    result = sorted(result.items(), key=lambda x: x[1], reverse=True)
    return result


def calculate_all_infomation_gain_ratio(dataset: pd.DataFrame, label_index: int):
    columns = dataset.columns[:-1]
    result = {}
    for i in columns:
        result[i] = calculate_information_gain(
            dataset, i, dataset.columns[label_index]) / Empirical_Entropy.calculate_empirical_entropy_of_a_column(dataset, i)
    result = sorted(result.items(), key=lambda x: x[1], reverse=True)
    return result
