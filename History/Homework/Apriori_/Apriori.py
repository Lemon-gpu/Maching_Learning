...
def clean_data(data: pd.DataFrame, min_support: int):
    frequency = {}
    for i in range(0, data.shape[0]):
...
def digging_itemset(item_set: pd.DataFrame, data: pd.DataFrame, min_support: int) -> list:
...
def getting_frequent_itemset(filePath: str, min_sup: int):
    ...
    item_set, raw = clean_data(data, min_support)
    result = digging_frequent_itemset(
        pd.DataFrame(item_set), data, min_support)
    raw_result = digging_itemset(
        pd.DataFrame(raw), data, min_support)
    result.insert(0, item_set)
    raw_result.insert(0, raw)
    return result, raw_result


def getting_maximins_frequent_itemset(filePath: str, min_support: int):
    temp_result, a = getting_frequent_itemset(filePath, min_support)
    temp_result.reverse()
